class_name LightDetectionHandler
extends SubViewport
# Determines current light level and visibility of a target Node3D.


@export var camera: Camera3D
@export var dark_adaptation_light: OmniLight3D
@export var detection_mesh: MeshInstance3D
...
@onready var color_rect: ColorRect = $"../ui/ColorRect" # TODO: TEMP
...
func _ready() -> void:
	...
	camera.set_cull_mask_value(GameObjects.render_layers.LIGHT_DETECTION, true)
	
	detection_mesh.set_layer_mask_value(GameObjects.render_layers.WORLD, false)
	detection_mesh.set_layer_mask_value(GameObjects.render_layers.LIGHT_DETECTION, true)
	
	update_timer.timeout.connect(Callable(on_update_timer_timeout))


# Average all pixel colors into one and return the resulting Color.
...
# Get average color and update light level.

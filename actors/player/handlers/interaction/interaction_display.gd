...
var current_display := CurrentDisplay.NONE:
	set(next_display):
		if current_display == next_display: return
		match current_display:
			CurrentDisplay.INTERACT:
				dot.hide()
				circle.hide()
			CurrentDisplay.LOCK:
				locked.hide()
...
func _ready() -> void:
	...
	interact_timer.timeout.connect(Callable(on_interact_timer_timeout))


func interact_display(value: bool) -> void:
	if value:
		current_display = CurrentDisplay.INTERACT
...
func light_display(is_on: bool) -> void:
	if is_on:
		...
		light_on.hide()
		light_off.show()
	current_display = CurrentDisplay.LIGHT
...
func lock_display(is_locked: bool) -> void:
	if is_locked: # Door is locked.
		...
		locked.hide()
		unlocked.show()
	current_display = CurrentDisplay.LOCK


func on_interact_timer_timeout() -> void:
...
func use_key_display(has_key: bool, is_locked: bool) -> void:
	if not has_key: no_key.show()
		...
		lock_door.show()
		unlock_door.hide()
	current_display = CurrentDisplay.USE_KEY
